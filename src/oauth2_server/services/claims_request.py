"""OIDC Core §5.5 `claims` request — the id_token half (divergence 59).
...
The `userinfo` member is NOT honored (that half is still a backlog item), and
`claims_parameter_supported` stays unadvertised in discovery.
"""
...
__all__ = ["ClaimsSelection", "select_id_token_claims"]
...
def select_id_token_claims(claims_request: str | None, *, scope: str) -> ClaimsSelection:
    """Build the `ClaimsSelection` for a stored `claims` parameter.
    ...
    if not isinstance(document, dict):
        return ClaimsSelection()
    members = document.get("id_token")
    if not isinstance(members, dict):
        return ClaimsSelection()

    scope_set = set(scope.split())
    selection = ClaimsSelection()
    for name, required_scope in _SUPPORTED.items():
